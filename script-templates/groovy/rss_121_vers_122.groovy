/**:encoding=UTF-8:
 * Correction de RSS de version 121 en version 122
...
rss {
  name 'Conversion des RSS de v.121 en v.122'
  ...
  onInit {
	...
	metaRSS122 = mfl.loadMeta('/rss122.csv') //récupérer les métadonnées pour un rss de type 122
  }
  ...
  onItem {item->
	...
	def nadl = rum.txtNADL
	def dshop = dateSortieHopParNadl[nadl]
	if (vrss == '121' && vrum == '021' && (dshop == premierMars || dshop.after(premierMars))) {
	    //créer un nouveau RSS format 122
		def sb = new StringBuffer()
		//rhswr.writeRhs(rhs, sb)
		//sb << "\r\n"
		def rum_tr = metaRSS122.getFirstChildGroupMeta().makeNewNode() //créer un RSS vide de type RSS122
		rum.copyFieldsTo(rum_tr) //faire la recopie de tous les champs qui existent dans l'ancienne version
		rum_tr.VRSS.value = '122' //changer valeur pour la version "RSS"
		rum_tr.VRUM.value = '022' //changer valeur pour la version de "RUM"
