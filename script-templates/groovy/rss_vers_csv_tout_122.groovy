// :encoding=utf-8:
/**
 * Exporter tous les champs du RSS v.122 vers un fichier csv
 * arguments :
 * -a:input
 * -a:output
 ...
 * Exemple d'exécution :
 * c:\app\gpmsi\v@PROJECT_VERSION@\gpmsi.bat -script c:\app\gpmsi\v@PROJECT_VERSION@\scripts\groovy\rss_vers_csv_tout_122.groovy -a:input MCO_RSSG_20250226145926.txt -a:output MCO_RSSG_20250226145926.csv
 *
...
//"souligner" les espaces (remplace les espaces par _)
def sesp(String s) { s?.replace(' ' as char, '_' as char) }
...
single {
  ...
  onInit {
      ...
      outp.f 'gvc'    //Groupage : version de la classification
      outp.f 'ncmd'   //Groupage : n° de GHM N° CMD
      outp.f 'nghm'   //N° GHM
      outp.f 'flr009' //Filler
      outp.f 'vrss'   //N° de version du format de RSS
      outp.f 'gcr'    //Groupage : code retour
      outp.f 'finess' //Numéro FINESS d’inscription ePMSI
      outp.f 'vrum'   //Version du format du RUM
      ...
      outp.f 'tald'   //Type d'autorisation du lit dédié
      ...
      outp.f 'cpre'   //Code postal de résidence (ou 99 suivi du code Insee du pays pour les patients résidant hors de France)
      outp.f 'pnne'   //Poids du nouveau-né à l'entrée de l'unité médicale (en grammes)
      outp.f 'ageg'   //Age gestationnel
      outp.f 'ddr'    //Date des dernières règles
      ...
      outp.f 'igs2'   //IGS2
      outp.f 'ccrs'   //Confirmation du codage du RSS
      outp.f 'tyma'   //Type de machine en radiothérapie
      outp.f 'tydo'   //Type de dosimétrie
      outp.f 'numi'   //Numéro d'innovation
      outp.f 'convhc' //Conversion hospitalisation complète
      outp.f 'pecraac' //Prise en charge RAAC
      outp.f 'ctxpsp' //Contexte patient/Surveillance particulière
      outp.f 'admprh' //Administration d'un produit de la RH
      outp.f 'rescrt' //Rescrit tarifaire
      outp.f 'catnit' //Catégorie du nombre d'interventions totales
      outp.f 'flr186' //Filler
      outp.f 'zr190' //Zone réservée
      //outp.f 'nivg'
      //outp.f 'apivg'
      //outp.f 'nnva'
  ...
  onItem { item ->
    rums.each() {rum ->
        def gvc = rum.txtGVC
        outp.f "$gvc" //Groupage : version de la classification
        def ncmd = rum.txtNCMD
        outp.f "$ncmd" //Groupage : n° de GHM N° CMD
        def nghm = rum.txtNGHM
        outp.f "$nghm" //N° GHM
        def flr009 = rum.txtFLR009
        outp.f "$flr009" //Filler
        def vrss = rum.txtVRSS
        outp.f "$vrss" //N° de version du format de RSS
        def gcr = rum.txtGCR
        outp.f "$gcr" //Groupage : code retour
        def finess = rum.txtFINESS
        outp.f "$finess" //Numéro FINESS d’inscription ePMSI
        def vrum = rum.txtVRUM
        outp.f "$vrum" //Version du format du RUM
        def nadl = rum.txtNADL
        outp.f "$nadl" //envoyer nadl trimmé
        def nrss = rum.txtNRSS
        outp.f nrss
        ...
        outp.f numi
        def convhc = rum.txtCONVHC //Conversion hospitalisation complète
        outp.f convhc
        def pecraac = rum.txtPECRAAC //Prise en charge RAAC
        outp.f pecraac
        def ctxpsp = rum.txtCTXPSP //Contexte patient/Surveillance particulière
        outp.f ctxpsp
        def admprh = rum.txtADMPRH //Administration d'un produit de la RH
        outp.f admprh
        def rescrt = rum.txtRESCRT //Rescrit tarifaire
        outp.f rescrt
        def catnit = rum.txtCATNIT //Catégorie du nombre d'interventions totales
        outp.f catnit
        def flr186 = rum.txtFLR186 //Filler
        outp.f flr186
        def zr190 = rum.txtZR190 //Zone réservée
        outp.f zr190
        ...
        //ajout parcours de soins : mode entrée + provenance + passage par urg + liste des UMs + mode sortie + destination
        def parcours = []
        def listeRums = rumsParNrss[nrss]
        def nPremierRum = 0
        def nDernierRum = listeRums.size()-1
        def entree = ""
        def sortie = ""
        listeRums.eachWithIndex {r,i ->
            if (i == nPremierRum) entree = r.MEUM.value + r.PROV.value + r.PSUR.value
            parcours << r.txtNUM
            if (i == nDernierRum) sortie = r.MSUM.value + r.DEST.value
        }
        outp.f sesp(entree) + "[" + parcours*.trim().join('-') + "]" + sesp(sortie)
        ...
        def nghm_v = rum.NGHM.value
        outp.f nghm_v[0]    //categorie
        outp.f nghm_v[1..2] //racine de GHM
        outp.f nghm_v[3]    //severite
        outp.endRow()
    }// rums.each
  }
  onEnd {
