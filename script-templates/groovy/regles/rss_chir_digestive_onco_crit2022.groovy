/**☺:encoding=UTF-8:
 * Dans un fichier de RSS,
 * recherche des RUMs qui concernent de la chirurgie Oncologique critères INCA 2022
...
ghmC = new GhmRumCodePresence('..C...')

/** Critère âge est 18 ans et plus */
age18 = new GenericPmsiCriterion( {context->
    def rum = context.rum
    def dnais = rum.DNAIS.toDate()
    def deum = rum.DEUM.toDate() //normalement il faudrait plutot la date d'entrée dans l'hôpital, à améliorer plus tard
    def age = DateUtils.calcAge(dnais, deum).getYears()
...
/** Critère diagnostic principal du RUM est un des DP de la liste */
estCimChirDig = new CimCodePresence('DP', CIM10_tumeur_maligne_digestif) //est-ce que le diag est present dans le DP du RUM ?
...
rss {
    ...
    onItem {item->
        ...
        if (n > 0 && nrssEnvoyes.contains(nrss) == false) {
            outf << "$nrss;${nadl}\r\n"
            nrssEnvoyes << nrss
        }
    }
