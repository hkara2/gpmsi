/**☺:encoding=UTF-8:
 * Recherche des RSAs qui concernent les actes de la chirurgie Oncologique critères INCA 2023,
 * mais avec un mauvais DP.
 *
 ...
 * c:\app\gpmsi\v1.3\gpmsi -script c:\app\gpmsi\v1.3\scripts\groovy\regles\rsa_chir_digestive_onco_crit2023_pb_dp.groovy -a:input 910019447.2022.10.rsa -a:output NRSAs_chir_dig_onco.txt
 */
package regles
 ...
 * ET NON(diagnostic principal spécifique de la chirurgie concernée par le seuil)
 * ET acte spécifique de la chirurgie concernée par le seuil
 *
 * Formule appelée "A1 et B1 (hors organes)"
 * ----------------------------
 * Âge supérieur ou égal à 18 ans
...
age18 = new GenericPmsiCriterion( {context->
    def rsa = context.rsa //recupere l'enregistrement rsa courant depuis le contexte
    def agea = rsa.AGEA.toInt()
    //println "age:$age"
    agea >= 18
})

//Codes INCA critères chir dig 2023 (par rapport à 2022, il y a juste le C786 qui a été barré sur le listing)
...
C785 C787 C788 D001 D002 D010 D011 D012 D013 D014 D015 D017 D371 D372 D373 D374
D375 D376 D377 D4830 D484
""").tokenize(' \n')

CIM10_tumeur_maligne_gynecologique = ("""
C510 C511 C512 C518 C519 C52 C530 C531 C538 C539 C540 C541 C542 C543 C548 C549
C55 C56 C570 C573 C571 C572 C574 C577 C578 C58 C796 D060 D061 D067 D069 D070
D071 D072 D073 D390 D391 D392 D397
""").tokenize(' \n')

CIM10_tumeur_maligne_metastase_peritoneale = ("""
C786
""").tokenize(' \n')

//Codes CCAM INCA 2023
...
estCimTumeurDig = new CimCodePresence('DPA', CIM10_tumeur_maligne_digestif) //est-ce que le diag est present dans le DP du RSA ?
...
/** Critere il y a au moins un DAS de tumeur maligne gynecologique */
estCimTumeurMaligneGyneco = new CimCodePresence('DAS', CIM10_tumeur_maligne_gynecologique) //est-ce qu'il y a au moins un DAS de la liste ?

criteres = new PmsiAnd(
  age18,
  ghmC,
  new PmsiNot( //on cherche justement les séjours qui n'ont pas ces critères, pour éventuelle réparation
      new PmsiOr(
        estCimTumeurDig,
        new PmsiAnd(
          // Critere au moins un des RUMs contient DP de métastase péritonéale
          new CimCodePresence('DP', CIM10_tumeur_maligne_metastase_peritoneale),
          new PmsiNot(estCimTumeurMaligneGyneco)
        )
      )
  ),
  estCcamExerese
